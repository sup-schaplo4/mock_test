"""
Floor-Based Puzzle Question Generator
...
# Question Metadata
TOPIC_NAME = "Floor Puzzle"
REASONING_TOPIC = "Floor Puzzle"
...
PROMPT_TEMPLATE = """# TASK: FLOOR-BASED PUZZLE QUESTION GENERATOR

## CONTEXT
You are an expert in logical reasoning, tasked with creating high-quality Floor-Based Puzzles. These puzzles involve arranging people or objects across different floors of a building based on a set of conditions and test skills in logical deduction, vertical spatial reasoning, and constraint satisfaction.

## YOUR TASK
Please generate {batch_size} Floor-Based Puzzle questions.
...
### EASY
**Puzzle Structure:**
- 5-6 floors in a single building
- 5-6 people/items (one per floor OR some floors empty)
- 4-5 direct conditions (e.g., "A lives on floor 3", "B lives immediately above C")
- Single-layer logic
- No complex interdependencies

### MEDIUM
**Puzzle Structure:**
- 6-8 floors in a single building
- 6-8 people/items with possible vacant floors
- 6-7 conditions with mix of direct and indirect clues (e.g., "between", "not adjacent")
- Two-layer logic
- May include additional attributes (color, profession, hobby)
- Moderate interdependencies

### HARD
**Puzzle Structure:**
- 8-10 floors OR two buildings with 5+ floors each
- 8-10 people/items with multiple attributes
- 8-10 conditions with multiple indirect/negative clues
- Three-layer logic
- Complex relationships (gaps, multiple floors between, symmetry)
- High interdependencies requiring multi-step deduction

## FLOOR CONVENTIONS
- Floors are numbered from bottom to top (Floor 1 = Ground/Bottom, Floor N = Top)
- Clearly state if ground floor is Floor 0 or Floor 1
- Use consistent floor numbering throughout each puzzle
...
Start from ID: floor_puzzle_{start_id:03d}
...
{{
  "questions": [
    {{
      ...
      "correct_answer": "A",
      "explanation": "[Detailed step-by-step solution with final floor arrangement]",
      "difficulty": "Easy",
      "topic": "{topic}",
...
def generate_floor_puzzle_questions():
    """
    Main function to generate floor-based puzzle questions
    """
    
    print("=" * 80)
    print("FLOOR-BASED PUZZLE QUESTION GENERATOR")
    print("=" * 80)
    ...
    output_file = os.path.join(OUTPUT_BASE_DIR, f"floor_puzzle_questions_{timestamp}.json")
    error_log_file = os.path.join(OUTPUT_BASE_DIR, f"floor_puzzle_errors_{timestamp}.log")
    metadata_file = os.path.join(OUTPUT_BASE_DIR, f"floor_puzzle_metadata_{timestamp}.json")
        ...
        if result["success"]:
            ...
            if validation["valid"]:
                ...
                for idx, q in enumerate(questions):
                    expected_id = f"floor_puzzle_{start_id + idx:03d}"
...
if __name__ == "__main__":
    generate_floor_puzzle_questions()
