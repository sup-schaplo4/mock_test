"""
Box-Based Puzzle Question Generator
...
# Question Metadata
TOPIC_NAME = "Box Puzzle"
REASONING_TOPIC = "Box Puzzle"
...
PROMPT_TEMPLATE = """# TASK: BOX-BASED PUZZLE QUESTION GENERATOR

## CONTEXT
You are an expert in logical reasoning, tasked with creating high-quality Box-Based Puzzles. These puzzles involve arranging boxes or items in a stack/sequence based on a set of conditions and test skills in logical deduction, positional reasoning, and constraint satisfaction.

## YOUR TASK
Please generate {batch_size} Box-Based Puzzle questions.
...
### EASY
**Puzzle Structure:**
- 5-6 boxes stacked vertically OR arranged in a single row
- 5-6 items (fruits, colors, objects) with single attributes
- 4-5 direct conditions (e.g., "Red box is at position 3", "Apple box is immediately above Banana box")
- Single-layer logic
- No complex interdependencies

### MEDIUM
**Puzzle Structure:**
- 6-8 boxes in vertical stack OR horizontal arrangement
- 6-8 items with 2 attributes (e.g., color + fruit, or number + size)
- 6-7 conditions with mix of direct and indirect clues (e.g., "between", "not adjacent", "exactly two boxes apart")
- Two-layer logic
- Moderate interdependencies between attributes

### HARD
**Puzzle Structure:**
- 8-10 boxes OR multiple stacks/shelves (e.g., 2 shelves with 5 boxes each)
- 8-10 items with 3+ attributes (e.g., color + content + weight/size)
- 8-10 conditions with multiple indirect/negative clues
- Three-layer logic
- Complex relationships (gaps, conditional dependencies, symmetry)
- High interdependencies requiring multi-step deduction

## BOX ARRANGEMENT TYPES
- **Vertical Stack**: Boxes stacked from bottom to top (Position 1 = Bottom, Position N = Top)
- **Horizontal Row**: Boxes arranged left to right (Position 1 = Leftmost, Position N = Rightmost)
- **Shelf System**: Multiple rows/shelves with boxes
- Clearly state the arrangement type and numbering convention
...
Start from ID: box_puzzle_{start_id:03d}
...
{{
  "questions": [
    {{
      ...
      "correct_answer": "A",
      "explanation": "[Detailed step-by-step solution with final box arrangement]",
      "difficulty": "Easy",
      "topic": "{topic}",
...
def generate_box_puzzle_questions():
    """
    Main function to generate box-based puzzle questions
    """
    
    print("=" * 80)
    print("BOX-BASED PUZZLE QUESTION GENERATOR")
    print("=" * 80)
    ...
    output_file = os.path.join(OUTPUT_BASE_DIR, f"box_puzzle_questions_{timestamp}.json")
    error_log_file = os.path.join(OUTPUT_BASE_DIR, f"box_puzzle_errors_{timestamp}.log")
    metadata_file = os.path.join(OUTPUT_BASE_DIR, f"box_puzzle_metadata_{timestamp}.json")
        ...
        if result["success"]:
            ...
            if validation["valid"]:
                ...
                for idx, q in enumerate(questions):
                    expected_id = f"box_puzzle_{start_id + idx:03d}"
