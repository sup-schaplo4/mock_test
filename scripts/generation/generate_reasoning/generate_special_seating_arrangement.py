"""
Special Seating Arrangement Question Generator (Square/Rectangular/Triangular/Hexagonal)
...
# Question Metadata
TOPIC_NAME = "Special Seating"
REASONING_TOPIC = "Special Seating"
...
PROMPT_TEMPLATE = """# TASK: SPECIAL SEATING ARRANGEMENT QUESTION GENERATOR

## CONTEXT
You are an expert in logical reasoning, tasked with creating high-quality Special Seating Arrangement puzzles. These puzzles involve arranging people or objects in special geometric configurations (square, rectangular, triangular, hexagonal tables) based on a set of conditions and test skills in logical deduction, spatial reasoning, and constraint satisfaction.

## YOUR TASK
Please generate {batch_size} Special Seating Arrangement questions.
...
### EASY
**Puzzle Structure:**
- Square table with 4 sides (1 person per side) OR small rectangular table
- 4-5 direct conditions (e.g., "A sits on the north side", "B sits opposite to C")
- Single-layer logic
- All facing center OR all facing outside (consistent direction)
- No complex interdependencies

### MEDIUM
**Puzzle Structure:**
- Square table with 8 people (2 per side) OR rectangular table OR triangular table
- 6-7 conditions with mix of direct and indirect clues
- Two-layer logic
- May include mixed facing directions or corner/middle positions
- Moderate interdependencies

### HARD
**Puzzle Structure:**
- Large square/rectangular table (12+ positions) OR hexagonal table OR complex multi-table setup
- 8-10 conditions with multiple indirect/negative clues
- Three-layer logic
- Mixed facing directions with complex neighbor, corner, and diagonal relationships
- High interdependencies requiring multi-step deduction

## TABLE TYPES TO USE
- **Square Tables**: 4 sides (North, South, East, West), people at corners and/or midpoints
- **Rectangular Tables**: 2 long sides, 2 short sides
- **Triangular Tables**: 3 sides/corners
- **Hexagonal Tables**: 6 sides/corners
- **Multiple Tables**: Combinations of above

## QUESTION ID FORMAT
Use this format for question IDs:
- special_seating_arrangement_001
- special_seating_arrangement_002
- And so on...

Start from ID: special_seating_arrangement_{start_id:03d}
...
{{
  "questions": [
    {{
      "question_id": "special_seating_arrangement_001",
      ...
      "correct_answer": "A",
      "explanation": "[Detailed step-by-step solution with final arrangement]",
      "difficulty": "Easy",
      "topic": "{topic}",
...
def generate_special_seating_questions():
    """
    Main function to generate special seating arrangement questions
    """
    
    print("=" * 80)
    print("SPECIAL SEATING ARRANGEMENT QUESTION GENERATOR")
    print("=" * 80)
    ...
    output_file = os.path.join(OUTPUT_BASE_DIR, f"special_seating_questions_{timestamp}.json")
    error_log_file = os.path.join(OUTPUT_BASE_DIR, f"special_seating_errors_{timestamp}.log")
    metadata_file = os.path.join(OUTPUT_BASE_DIR, f"special_seating_metadata_{timestamp}.json")
        ...
        if result["success"]:
            ...
            if validation["valid"]:
                ...
                for idx, q in enumerate(questions):
                    expected_id = f"special_seating_arrangement_{start_id + idx:03d}"
...
if __name__ == "__main__":
    generate_special_seating_questions()
