"""
Scheduling/Day-Month Puzzle Question Generator
...
# Question Metadata
TOPIC_NAME = "Day/Month Puzzle"
REASONING_TOPIC = "Scheduling Puzzle"
...
PROMPT_TEMPLATE = """# TASK: SCHEDULING/DAY-MONTH PUZZLE QUESTION GENERATOR

## CONTEXT
You are an expert in logical reasoning, tasked with creating high-quality Scheduling and Day-Month Puzzles. These puzzles involve arranging events, activities, or people across days of the week, months, dates, or time slots based on a set of conditions and test skills in temporal logic, sequencing, and constraint satisfaction.

## YOUR TASK
Please generate {batch_size} Scheduling/Day-Month Puzzle questions.
...
### EASY
**Puzzle Structure:**
- 5-6 events/people scheduled across 5-6 consecutive days OR single week (Mon-Fri/Sat)
- Single attribute (e.g., only days OR only dates)
- 4-5 direct conditions (e.g., "Meeting on Wednesday", "Event X is before Event Y")
- Single-layer logic
- No complex interdependencies

### MEDIUM
**Puzzle Structure:**
- 6-8 events/people across 6-8 time slots (e.g., week days, specific dates, or months)
- Two attributes (e.g., day + time slot, OR date + month)
- 6-7 conditions with mix of direct and indirect clues (e.g., "gap of 2 days", "same week", "not on consecutive days")
- Two-layer logic
- Moderate interdependencies

### HARD
**Puzzle Structure:**
- 8-10 events/people across complex temporal framework (e.g., specific dates across multiple months, multiple weeks, or day+time combinations)
- Three attributes (e.g., day + date + month, OR person + activity + time slot)
- 8-10 conditions with multiple indirect/negative clues and temporal relationships
- Three-layer logic
- Complex patterns (alternate days, specific gaps, quarterly, conditional scheduling)
- High interdependencies requiring multi-step deduction

## TEMPORAL FRAMEWORKS
- **Days of Week**: Monday through Sunday (or specific subset)
- **Dates & Months**: Specific dates (e.g., 5th, 10th, 15th, 20th, 25th) across months
- **Time Slots**: Morning/Afternoon/Evening, or specific hours
- **Mixed**: Combinations like "Wednesday 5th March" or "2 PM on Friday"
- Clearly specify the temporal framework and any relevant constraints (e.g., consecutive dates, same month)

## QUESTION ID FORMAT
Use this format for question IDs:
- scheduling_puzzle_001
- scheduling_puzzle_002
- And so on...

Start from ID: scheduling_puzzle_{start_id:03d}
...
{{
  "questions": [
    {{
      "question_id": "scheduling_puzzle_001",
      ...
      "correct_answer": "A",
      "explanation": "[Detailed step-by-step solution with final scheduling arrangement]",
      "difficulty": "Easy",
      "topic": "{topic}",
...
def generate_scheduling_puzzle_questions():
    """
    Main function to generate scheduling/day-month puzzle questions
    """
    
    print("=" * 80)
    print("SCHEDULING/DAY-MONTH PUZZLE QUESTION GENERATOR")
    print("=" * 80)
    ...
    output_file = os.path.join(OUTPUT_BASE_DIR, f"scheduling_puzzle_questions_{timestamp}.json")
    error_log_file = os.path.join(OUTPUT_BASE_DIR, f"scheduling_puzzle_errors_{timestamp}.log")
    metadata_file = os.path.join(OUTPUT_BASE_DIR, f"scheduling_puzzle_metadata_{timestamp}.json")
        ...
        if result["success"]:
            ...
            if validation["valid"]:
                ...
                for idx, q in enumerate(questions):
                    expected_id = f"scheduling_puzzle_{start_id + idx:03d}"
...
if __name__ == "__main__":
    generate_scheduling_puzzle_questions()
