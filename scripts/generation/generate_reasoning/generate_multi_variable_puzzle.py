"""
Multi-Variable/Complex Puzzle Question Generator
...
# Question Metadata
TOPIC_NAME = "Complex Puzzle"
REASONING_TOPIC = "Multi-Variable Puzzle"
...
PROMPT_TEMPLATE = """# TASK: MULTI-VARIABLE/COMPLEX PUZZLE QUESTION GENERATOR

## CONTEXT
You are an expert in logical reasoning, tasked with creating high-quality Multi-Variable Complex Puzzles. These puzzles involve multiple entities (people, objects, events) with multiple attributes (colors, professions, floors, days, preferences, etc.) and test advanced skills in logical deduction, constraint satisfaction, and multi-dimensional reasoning.

## YOUR TASK
Please generate {batch_size} Multi-Variable Complex Puzzle questions.

**Key Guidelines for Solvability:**
As an expert puzzle creator, your primary goal is to ensure every puzzle is fair and solvable. Please follow this internal verification process for each question you create:
1. First, ensure all conditions are logically consistent and lead to a single, unique solution.
2. Second, solve the puzzle step-by-step internally to confirm its solvability.
3. Finally, verify that the correct answer and the explanation are fully supported by your logical deduction.
4. Please ensure all generated puzzles are solvable, unambiguous, and logically consistent.
...
### EASY
**Puzzle Structure:**
- 5 entities (people/objects)
- 2-3 attributes per entity (e.g., Name + Color + Floor)
- 5-6 direct conditions with minimal cross-attribute dependencies
- Single-layer logic with clear deductions
- No complex interdependencies

### MEDIUM
**Puzzle Structure:**
- 6-7 entities
- 3-4 attributes per entity (e.g., Name + Profession + Floor + Favorite Color)
- 7-8 conditions with mix of direct and indirect clues
- Two-layer logic requiring intermediate deductions
- Moderate cross-attribute dependencies (e.g., "The doctor lives above the person who likes blue")

### HARD
**Puzzle Structure:**
- 8-10 entities
- 4-5 attributes per entity (e.g., Name + Profession + Floor + Day + Favorite Fruit + Car Color)
- 10-12 conditions with multiple indirect/negative clues and complex relationships
- Three-layer logic requiring extensive cross-referencing
- High interdependencies with compound conditions (e.g., "The person who visits on Monday lives two floors above the engineer who drives a red car")
- May include grouping, exclusion, or conditional logic

## PUZZLE FRAMEWORKS
**Common Structures:**
- Building/Floor arrangements with multiple attributes per resident
- Weekly schedules with person + activity + time + additional attributes
- Comparative arrangements (taller/shorter, earlier/later, more/less)
- Grid-based logic with multiple intersecting categories

**Attribute Examples:**
- Personal: Names, genders, ages, relationships
- Professional: Jobs, departments, salaries, experience
- Preferences: Colors, fruits, sports, hobbies, brands
- Spatial: Floors, positions, seats, houses
- Temporal: Days, months, time slots, order
- Physical: Car types, clothing colors, accessories

## QUESTION ID FORMAT
Use this format for question IDs:
- multivariable_puzzle_001
- multivariable_puzzle_002
- And so on...

Start from ID: multivariable_puzzle_{start_id:03d}
...
{{
  "questions": [
    {{
      "question_id": "multivariable_puzzle_001",
      "question": "[Full puzzle setup + question text]",
      "options": {{
        "A": "[Option A text]",
        "B": "[Option B text]",
        "C": "[Option C text]",
      ...
      "correct_answer": "A",
      "explanation": "[Detailed step-by-step solution showing how each attribute is determined]",
      "difficulty": "Easy",
      "topic": "{topic}",
...
def generate_multivariable_puzzle_questions():
    """
    Main function to generate multi-variable complex puzzle questions
    """
    
    print("=" * 80)
    print("MULTI-VARIABLE/COMPLEX PUZZLE QUESTION GENERATOR")
    print("=" * 80)
    ...
    output_file = os.path.join(OUTPUT_BASE_DIR, f"multivariable_puzzle_questions_{timestamp}.json")
    error_log_file = os.path.join(OUTPUT_BASE_DIR, f"multivariable_puzzle_errors_{timestamp}.log")
    metadata_file = os.path.join(OUTPUT_BASE_DIR, f"multivariable_puzzle_metadata_{timestamp}.json")
        ...
        if result["success"]:
            ...
            if validation["valid"]:
                ...
                for idx, q in enumerate(questions):
                    expected_id = f"multivariable_puzzle_{start_id + idx:03d}"
...
if __name__ == "__main__":
    generate_multivariable_puzzle_questions()
